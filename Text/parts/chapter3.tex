...
\begin{enumerate}
    ...
    \item \textbf{Средства языка C#: ref struct, in, out}: средства языка позволяющие работать с данными на стеке, избегая аллокаций в управляемой куче и обеспечивая безопасность типов
    \item \textbf{BenchmarkDotNet}: индустриальный стандарт для бенчмаркинга в экосистеме .NET, используемый для точного измерения времени выполнения криптографических операций с изоляцией влияния JIT-компиляции и сборщика мусора.
\end{enumerate}
...
Структура элементов поля, которая использует парадигму SoA (Structure of Arrays), \texttt{FieldElement4} 
спроектирована как \texttt{ref struct}, что гарантирует ее размещение исключительно на стеке. 
...
\begin{enumerate}   
    \item \textbf{Эффективность SIMD-векторизации.} Применение пакетной обработки (AVX2 Batched) обеспечивает качественный скачок в производительности. Время, затрачиваемое процессором на одну операцию в пакете из 4-х задач, сокращается практически вдвое по сравнению со скалярной версией Bouncy Castle. Разработанное ядро с поддержкой AVX2 превосходит индустриальный стандарт на \textbf{$\sim 30\% - \sim 80\%$}.
    \item \textbf{Отсутсвие выделений памяти в куче.} Применение паттернов Zero Allocation (использование \texttt{ref struct}, \texttt{stackalloc} и \texttt{Span<byte>}) позволило полностью исключить работу сборщика мусора, для выполнения всех опреаций.
